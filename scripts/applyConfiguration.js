...
/**
 * Set Tealeaf configuration values from cmd arguments to configuration 'TealeafConfig.json' file.
 * 
...
const filePath = "../../TealeafConfig.json"
...
function findCurrentConfigValues(configObject) {
    if (!fs.existsSync(filePath)) {
        console.error("'TealeafConfig.json' file not found in root app folder.");
        return;
    }
    ...
    try {
        ...
        console.error("Permission denied: 'TealeafConfig.json' file");
...
function setConfigValuesToFile(configObject, currentConfigObject) {
    if (currentConfigObject) {
        if (currentConfigObject.Tealeaf[AppKey]) {
            log("AppKey already set: " + currentConfigObject.Tealeaf[AppKey]);
            if (configObject.AppKey) {
                log("Update AppKey to: " + configObject.AppKey);
                currentConfigObject.Tealeaf[AppKey] = configObject.AppKey;
            }
        }
        else {
        ...
        if (currentConfigObject.Tealeaf[PostMessageUrl]) {
            log("PostMessageUrl already set: " + currentConfigObject.Tealeaf[PostMessageUrl]);
            if (configObject.PostMessageUrl) {
                log("Update PostMessageUrl to: " + configObject.PostMessageUrl);
                currentConfigObject.Tealeaf[PostMessageUrl] = configObject.PostMessageUrl;
            }
        }
        else {
...
log("Acoustic Tealeaf SDK installed, Looking for Tealeaf config values: AppKey and PostMessageUrl");
