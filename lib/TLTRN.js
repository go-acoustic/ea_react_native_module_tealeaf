...
export default class TLTRN {
    ...
    static getBooleanConfigItemForKey = async (key) => {
        const result = await RNCxa.getBooleanConfigItemForKey(key, moduleName = 'Tealeaf');
        console.log(`${key} result: `, result);
    }
    
    static setBooleanConfigItemForKey = async (key, value) => {
        const setBooleanConfigItemForKey = Platform.select({
            'ios': RNCxa.setBooleanConfigItemForKey,
            'android': RNCxa.setConfigItem
        });
        
        const result = await setBooleanConfigItemForKey(key, value, moduleName = 'Tealeaf');
        console.log(`${key} result: `, result);
    }
    ...
    static logClickEvent = async (target, reactId, ariaLabel) => {
        if (Platform.OS === 'ios') {
            await RNCxa.logClickEvent(target, reactId);
        } else {
            //await RNCxa.logClickEvent(target, reactId, ariaLabel);
        }
    }
