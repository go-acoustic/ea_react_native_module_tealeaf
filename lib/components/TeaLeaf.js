...
const Tealeaf = ({ children }) => {
    ...
    const onStartShouldSetResponderCapture = useCallback((event) => {
        ...
        var ariaLabel = event._targetInst.memoizedProps.accessible !== undefined ? event._targetInst.memoizedProps.accessibilityLabel : null;
        TLTRN.logClickEvent(event.target._nativeTag, reactId, ariaLabel);
